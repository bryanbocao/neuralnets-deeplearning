...
def plot_error(baseline_error0s, baseline_error1s, mean_squared_error_per_epochs, lr, H, bs):
    ...
    epochs = []
    for i in range(len(mean_squared_error_per_epochs_y)):
        epochs.append(i)
    epochs_x = np.array(epochs)
    plt.plot(epochs_x, mean_squared_error_per_epochs_y, label = 'Network mean squared error')
...
train(n_data, o, lr = 0.05, H = 10, bs = 100)
